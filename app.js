...
const STORAGE_KEY = 'churchcare-v3-data';
const APP_VERSION = 6;
...
const seedData = {
  ...
  members: [
    { id: 'm1', name: 'Kwame Mensah', phone: '024 111 2233', dob: '1984-03-12', joinedDate: '2021-01-10', role: 'shepherd', shepherdId: null, priorityContact: true, notes: '' },
    { id: 'm2', name: 'Ruth Agyeman', phone: '055 312 4801', dob: '1990-07-22', joinedDate: '2022-06-05', role: 'shepherd', shepherdId: null, priorityContact: true, notes: '' },
    { id: 'm3', name: 'Daniel Osei', phone: '020 778 9012', dob: '1987-11-08', joinedDate: '2020-04-19', role: 'shepherd', shepherdId: null, priorityContact: false, notes: '' },
    { id: 'm4', name: 'Akosua Mensah', phone: '024 450 3021', dob: '1998-08-03', joinedDate: '2023-02-12', role: 'member', shepherdId: 'm1', priorityContact: true, notes: '' },
    { id: 'm5', name: 'Daniel Owusu', phone: '055 880 1189', dob: '1994-01-14', joinedDate: '2021-09-05', role: 'member', shepherdId: 'm1', priorityContact: false, notes: 'Works shifts and may occasionally travel.' },
    { id: 'm6', name: 'Grace Mensah', phone: '020 350 9077', dob: '1999-07-18', joinedDate: '2024-01-07', role: 'member', shepherdId: 'm2', priorityContact: true, notes: '' },
    { id: 'm7', name: 'John Asare', phone: '027 412 6711', dob: '1996-10-25', joinedDate: '2022-04-03', role: 'member', shepherdId: 'm2', priorityContact: false, notes: '' },
    { id: 'm8', name: 'Esther Boateng', phone: '050 441 2288', dob: '2001-07-30', joinedDate: '2025-01-12', role: 'member', shepherdId: 'm3', priorityContact: true, notes: '' },
    { id: 'm9', name: 'Michael Osei', phone: '054 209 7305', dob: '1997-05-19', joinedDate: '2023-07-09', role: 'member', shepherdId: 'm3', priorityContact: false, notes: '' },
    { id: 'm10', name: 'Priscilla Addo', phone: '059 771 6030', dob: '2002-12-02', joinedDate: '2026-06-28', role: 'member', shepherdId: 'm1', priorityContact: false, notes: 'Recently joined.' },
    { id: 'm11', name: 'Samuel Kusi', phone: '053 609 4882', dob: '1995-09-14', joinedDate: '2024-05-19', role: 'member', shepherdId: null, priorityContact: false, notes: '' },
    { id: 'm12', name: 'Abigail Frimpong', phone: '024 771 6290', dob: '2000-02-28', joinedDate: '2025-03-02', role: 'member', shepherdId: 'm2', priorityContact: false, notes: '' }
  ],
  visitors: [
    { id: 'v1', name: 'Nana Yaa Serwaa', phone: '054 880 4410', dob: '2001-09-21', firstVisit: '2026-07-12', lastVisit: '2026-07-12', visits: 1, invitedByName: 'Akosua Mensah', invitedByMemberId: 'm4' },
    { id: 'v2', name: 'Kofi Boadu', phone: '024 730 1144', dob: '1998-04-06', firstVisit: '2026-07-05', lastVisit: '2026-07-12', visits: 2, invitedByName: 'Samuel Kusi', invitedByMemberId: 'm11' }
  ],
  services: [
    { id: 's1', date: '2026-05-24', service: 'Sunday Service', presentMemberIds: ['m1','m2','m3','m4','m5','m6','m7','m8','m9','m11','m12'], visitorIds: [] },
    { id: 's2', date: '2026-05-31', service: 'Sunday Service', presentMemberIds: ['m1','m2','m3','m4','m5','m6','m7','m8','m9','m11','m12'], visitorIds: [] },
    { id: 's3', date: '2026-06-07', service: 'Sunday Service', presentMemberIds: ['m1','m2','m3','m4','m5','m6','m7','m8','m9','m11','m12'], visitorIds: [] },
    { id: 's4', date: '2026-06-14', service: 'Sunday Service', presentMemberIds: ['m1','m2','m3','m4','m5','m6','m7','m8','m9','m11','m12'], visitorIds: [] },
    { id: 's5', date: '2026-06-21', service: 'Sunday Service', presentMemberIds: ['m1','m2','m3','m4','m6','m7','m8','m9','m11','m12'], visitorIds: [] },
    { id: 's6', date: '2026-06-28', service: 'Sunday Service', presentMemberIds: ['m1','m2','m3','m4','m6','m7','m8','m9','m10','m11','m12'], visitorIds: [] },
    { id: 's7', date: '2026-07-05', service: 'Sunday Service', presentMemberIds: ['m1','m2','m3','m4','m6','m7','m9','m10','m11','m12'], visitorIds: ['v2'] },
    { id: 's8', date: '2026-07-08', service: 'Midweek Service', presentMemberIds: ['m1','m2','m3','m4','m6','m7','m10','m12'], visitorIds: [] },
    { id: 's9', date: '2026-07-12', service: 'Sunday Service', presentMemberIds: ['m1','m2','m3','m4','m7','m10','m11','m12'], visitorIds: ['v1','v2'] }
  ],
  careNotes: []
};

let state = loadState();
let attendanceDraft = { presentMemberIds: new Set(), visitorIds: new Set() };
...
function normalizeVisitor(visitor) {
  return { visitorStatus: 'active', archivedDate: null, invitedByName: '', invitedByMemberId: null, ...visitor };
}
...
function normalizeLoadedState(parsed) {
  return {
    ...
    ...parsed,
    version: APP_VERSION,
    settings: { ...clone(seedData.settings), ...(parsed?.settings || {}) },
...
function loadState() {
  try {
    ...
    console.error('Could not load ChurchCare data:', error);
...
function getSundayRecords() {
  return state.services
    .filter(record => record.service === 'Sunday Service')
...
function getLatestServiceRecord() {
  return [...state.services].sort((a, b) => b.date.localeCompare(a.date))[0] || null;
}
...
function renderDashboard() {
  ...
  const sundayRecords = getSundayRecords().slice(0, 8).reverse();
...
function renderVisitors() {
  ...
  const archivedVisitors = state.visitors.length - activeVisitors.length;
  ...
  }).join('') : '<tr><td colspan="9"><div class="empty-state">No visitors match the current filters.</div></td></tr>';
}
...
function renderFilterOptions() {
  ...
  if (historyServiceFilter) {
    ...
    const services = [...new Set(state.services.map(record => record.service))].sort();
    historyServiceFilter.innerHTML = `<option value="all">All services</option>${services.map(service => `<option value="${escapeHTML(service)}">${escapeHTML(service)}</option>`).join('')}`;
...
function currentAttendanceKey() {
  return {
    date: document.getElementById('attendanceDate')?.value || isoToday(),
    service: document.getElementById('serviceSelect')?.value || 'Sunday Service'
  };
}

function loadAttendanceDraft() {
  const { date, service } = currentAttendanceKey();
  const existing = state.services.find(record => record.date === date && record.service === service);
  attendanceDraft = {
...
function saveAttendanceRecord() {
  const { date, service } = currentAttendanceKey();
  if (!date || !service) return showToast('Please choose a date and service.');

  let record = state.services.find(item => item.date === date && item.service === service);
  if (!record) {
    record = { id: uid('service'), date, service, presentMemberIds: [], visitorIds: [] };
  ...
  record.visitorIds.forEach(id => {
    ...
    if (!visitor) return;
    const visits = state.services.filter(serviceRecord => serviceRecord.visitorIds?.includes(id)).length;
    visitor.visits = visits;
    visitor.firstVisit = visitor.firstVisit || date;
    visitor.lastVisit = [...state.services].filter(serviceRecord => serviceRecord.visitorIds?.includes(id)).sort((a, b) => b.date.localeCompare(a.date))[0]?.date || date;
  });

  saveState();
  renderAll();
  const stats = getRecordStats(record);
  showToast(`Attendance saved: ${stats.present} members + ${stats.visitors} visitors = ${stats.total} total.`);
}
...
function renderHistory() {
  ...
  const records = [...state.services]
    .filter(record => `${record.date} ${record.service} ${formatDate(record.date)}`.toLowerCase().includes(query))
    .filter(record => serviceFilter === 'all' || record.service === serviceFilter)
    .sort((a, b) => b.date.localeCompare(a.date));

  body.innerHTML = records.length ? records.map(record => {
    const stats = getRecordStats(record);
    return `<tr>
      <td>${formatDate(record.date)}</td><td>${escapeHTML(record.service)}</td><td>${stats.present}</td><td>${stats.absent}</td><td>${stats.visitors}</td><td><strong>${stats.total}</strong></td><td><strong>${stats.rate}%</strong></td><td><button class="row-action" data-history-open="${record.id}">Open</button></td>
    </tr>`;
  }).join('') : '<tr><td colspan="8"><div class="empty-state">No attendance records match the current filters.</div></td></tr>';
}

function renderCare() {
  const careMembers = getCareMembers();
  document.getElementById('classificationRuleText').textContent = `Active = at least ${state.settings.activeThreshold}% of the latest ${state.settings.windowSize} Sunday services, with fewer than ${state.settings.inactiveConsecutive} consecutive absences.`;
...
function renderReports() {
  const sundayRecords = getSundayRecords().slice(0, 8).reverse();
  const allVisitors = getActiveVisitors().length;
  const activeCount = getCurrentMembers().filter(member => getMemberMetrics(member).status === 'active').length;
  const inactiveCount = getCurrentMembers().filter(member => getMemberMetrics(member).status === 'inactive').length;
...
function openModal({ eyebrow = 'ChurchCare', title, subtitle = '', body, wide = false, onOpen = null }) {
    ...
    onOpen: () => {
      ...
      form.addEventListener('submit', event => {
        ...
        if (type === 'member') {
          ...
          if (!visitor) {
            visitor = normalizeVisitor({ id: uid('visitor'), name, phone, dob, firstVisit: date, lastVisit: date, visits: 0, invitedByName, invitedByMemberId: findInviterMemberId(invitedByName) });
    ...
    onOpen: () => {
      document.getElementById('convertVisitorForm').addEventListener('submit', event => {
        ...
        const member = normalizeMember({
          ...
          joinedDate: visitor.firstVisit || visitor.lastVisit || isoToday(), role: 'member', shepherdId: String(data.get('shepherdId') || '') || null,
...
function renderSettings() {
  ...
  if (summary) summary.textContent = `Current members are classified Active when they attend at least ${state.settings.activeThreshold}% of the latest ${state.settings.windowSize} Sunday services and have fewer than ${state.settings.inactiveConsecutive} consecutive absences. Former or archived members are excluded automatically.`;
...
function openHistoryRecord(recordId) {
  ...
  openModal({
    ...
    wide: true,
    body: `<div class="history-detail-grid">
...
function exportMembersCsv() {
  ...
  downloadFile(`churchcare-members-${isoToday()}.csv`, rows.map(row => row.map(csvEscape).join(',')).join('\n'), 'text/csv;charset=utf-8');
}


function exportVisitorsCsv() {
  const rows = [['Name','Phone','Invited By','Date of Birth','First Visit','Last Visit','Total Visits','Visitor Status']];
  state.visitors.forEach(visitor => rows.push([visitor.name, visitor.phone, visitor.invitedByName || '', visitor.dob, visitor.firstVisit, visitor.lastVisit, visitor.visits || 0, visitor.visitorStatus || 'active']));
  downloadFile(`churchcare-visitors-${isoToday()}.csv`, rows.map(row => row.map(csvEscape).join(',')).join('\n'), 'text/csv;charset=utf-8');
}

function exportAttendanceCsv() {
  const rows = [['Date','Service','Members Present','Members Absent','Visitors/New','Total Attendance','Member Attendance Rate']];
  [...state.services].sort((a, b) => b.date.localeCompare(a.date)).forEach(record => {
    const stats = getRecordStats(record);
    rows.push([record.date, record.service, stats.present, stats.absent, stats.visitors, stats.total, `${stats.rate}%`]);
  });
  downloadFile(`churchcare-attendance-${isoToday()}.csv`, rows.map(row => row.map(csvEscape).join(',')).join('\n'), 'text/csv;charset=utf-8');
}

function exportBackup() {
  downloadFile(`churchcare-backup-${isoToday()}.json`, JSON.stringify(state, null, 2), 'application/json');
}
...
function importBackup(file) {
  ...
  reader.onload = () => {
    try {
      ...
      loadAttendanceDraft();
      renderAll();
      showToast('ChurchCare backup restored successfully.');
    } catch (error) {
      console.error(error);
...
function deleteAllSiteData() {
  const firstConfirmation = confirm('Delete ALL ChurchCare site data from this browser? This will remove every member, visitor, attendance record, shepherd assignment, care note, birthday record, and roster record. This cannot be undone unless you already have a backup.');
  ...
  attendanceDraft = { presentMemberIds: new Set(), visitorIds: new Set() };
  bulkSelectedMemberIds.clear();
  saveState();
  loadAttendanceDraft();
  renderAll();
  switchPage('dashboard');
  showToast('All ChurchCare site data has been permanently deleted.');
}

function resetDemoData() {
  if (!confirm('Reset all local ChurchCare data back to the demo records? This cannot be undone unless you have exported a backup.')) return;
...
document.addEventListener('click', event => {
  const target = event.target.closest('button, [data-member-view], [data-member-care], [data-shepherd-open], [data-assign-shepherd], [data-history-open], [data-convert-visitor], [data-remove-today-visitor], [data-edit-member], [data-delete-member], [data-change-member-status], [data-archive-visitor], [data-restore-visitor], [data-delete-visitor], [data-modal-cancel]');
  if (!target) return;
