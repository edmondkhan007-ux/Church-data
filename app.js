...
function syncMobileNavigation(pageName) {
  const parentPage = {
    classification: 'shepherds',
    followup: 'shepherds',
    members: 'welfare',
    visitors: 'welfare',
    history: 'welfare'
  }[pageName] || pageName;
  document.querySelectorAll('[data-mobile-go]').forEach(button => button.classList.toggle('active', button.dataset.mobileGo === parentPage));
  if (mobileMoreBtn) mobileMoreBtn.classList.toggle('active', ['operations','attendance','programs','reports','settings'].includes(pageName));
}
...
function normalizeLoadedState(parsed) {
  ...
  return {
    ...
    treasureHunts: Array.isArray(parsed?.treasureHunts) ? parsed.treasureHunts : [],
    welfareRecords: Array.isArray(parsed?.welfareRecords) ? parsed.welfareRecords : []
  };
}
...
function switchPage(pageName) {
  ...
  document.querySelectorAll('.nav-item').forEach(nav => nav.classList.toggle('active', nav.dataset.section === desktopParent));
...
function getTreasureHuntsForPeriod() {
  return (state.treasureHunts || []).filter(recordMatchesPeriod).slice().sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')));
}

function renderTreasureHunt() {
  const statsTarget = document.getElementById('treasureStats');
  const tableTarget = document.getElementById('treasureHuntTableBody');
  if (!statsTarget || !tableTarget) return;
  const records = getTreasureHuntsForPeriod();
  const reached = records.reduce((sum, item) => sum + Number(item.peopleReached || 0), 0);
  const interested = records.reduce((sum, item) => sum + Number(item.interestedCount || 0), 0);
  const firstTimers = records.reduce((sum, item) => sum + Number(item.firstTimersCount || 0), 0);
  statsTarget.innerHTML = [
    statCard('Treasure Hunts', records.length, 'Recorded in the selected period', '✦'),
    statCard('People Reached', reached, 'Total outreach contacts reported', '↗'),
    statCard('Interested', interested, 'People marked interested', '♡'),
    statCard('First-Timer Outcomes', firstTimers, 'Reported first-timers from outreach', '+')
  ].join('');

  tableTarget.innerHTML = records.length ? records.map(item => {
    const semester = getSemester(item.semesterId) || getSemesterForDate(item.date);
    return `<tr>
      <td><strong>${formatDate(item.date)}</strong><small class="table-subtext">${escapeHTML(semesterFullLabel(semester))}</small></td>
      <td>${escapeHTML(item.location || '—')}</td>
      <td>${escapeHTML(item.scope || 'Campus')}</td>
      <td><strong>${Number(item.peopleReached || 0)}</strong></td>
      <td>${Number(item.interestedCount || 0)}</td>
      <td>${Number(item.firstTimersCount || 0)}</td>
      <td>${escapeHTML(item.coordinator || '—')}</td>
      <td><span class="table-note-clip">${escapeHTML(item.notes || item.strategy || '—')}</span></td>
      <td><div class="table-actions"><button class="mini-btn" data-treasure-edit="${item.id}">Edit</button><button class="mini-btn danger-mini" data-treasure-delete="${item.id}">Delete</button></div></td>
    </tr>`;
  }).join('') : '<tr><td colspan="9"><div class="empty-state">No Treasure Hunt record has been added for this period yet.</div></td></tr>';
}

function openTreasureHuntForm(recordId = null) {
  const existing = (state.treasureHunts || []).find(item => item.id === recordId) || null;
  const currentSemester = getCurrentSemester();
  openModal({
    eyebrow: 'Treasure Hunt · Evangelism',
    title: existing ? 'Edit Treasure Hunt record' : 'Record Treasure Hunt',
    subtitle: 'Record the outreach itself and the immediate evangelism outcomes. Detailed first-timer information can still be kept in the Visitors register.',
    body: `<form class="lux-form" id="treasureHuntForm">
      <div class="form-grid two-col">
        <label><span>Date *</span><input type="date" name="date" required value="${escapeHTML(existing?.date || isoToday())}"></label>
        <label><span>Location *</span><input name="location" required value="${escapeHTML(existing?.location || '')}" placeholder="e.g. KNUST campus / Ayeduase"></label>
        <label><span>Scope</span><select name="scope"><option ${existing?.scope === 'Campus' ? 'selected' : ''}>Campus</option><option ${existing?.scope === 'Wider community' ? 'selected' : ''}>Wider community</option><option ${existing?.scope === 'Online / digital' ? 'selected' : ''}>Online / digital</option><option ${existing?.scope === 'Other' ? 'selected' : ''}>Other</option></select></label>
        <label><span>Coordinator / team lead</span><input name="coordinator" value="${escapeHTML(existing?.coordinator || '')}" placeholder="Name"></label>
        <label><span>People reached</span><input type="number" min="0" name="peopleReached" value="${Number(existing?.peopleReached || 0)}"></label>
        <label><span>Interested persons</span><input type="number" min="0" name="interestedCount" value="${Number(existing?.interestedCount || 0)}"></label>
        <label><span>First-timers resulting</span><input type="number" min="0" name="firstTimersCount" value="${Number(existing?.firstTimersCount || 0)}"></label>
        <label><span>Semester</span><select name="semesterId">${semesterOptions(existing?.semesterId || currentSemester?.id || '')}</select></label>
      </div>
      <label><span>Evangelism strategy / approach</span><textarea name="strategy" rows="2" placeholder="What approach was used?">${escapeHTML(existing?.strategy || '')}</textarea></label>
      <label><span>Notes / follow-up observations</span><textarea name="notes" rows="3" placeholder="Important outcomes, follow-up plans or observations">${escapeHTML(existing?.notes || '')}</textarea></label>
      <div class="modal-actions"><button type="button" class="btn btn-soft" data-modal-cancel>Cancel</button><button type="submit" class="btn btn-gold">${existing ? 'Save changes' : 'Save Treasure Hunt'}</button></div>
    </form>`,
    onOpen: () => document.getElementById('treasureHuntForm')?.addEventListener('submit', event => {
      event.preventDefault();
      const data = new FormData(event.currentTarget);
      const date = String(data.get('date') || '');
      const semester = getSemester(String(data.get('semesterId') || '')) || getSemesterForDate(date);
      const payload = {
        date,
        location: String(data.get('location') || '').trim(),
        scope: String(data.get('scope') || 'Campus'),
        coordinator: String(data.get('coordinator') || '').trim(),
        peopleReached: Number(data.get('peopleReached') || 0),
        interestedCount: Number(data.get('interestedCount') || 0),
        firstTimersCount: Number(data.get('firstTimersCount') || 0),
        semesterId: semester?.id || '',
        academicYear: semester?.academicYear || '',
        calendarYear: date ? date.slice(0,4) : (semester?.calendarYear || ''),
        strategy: String(data.get('strategy') || '').trim(),
        notes: String(data.get('notes') || '').trim()
      };
      if (existing) Object.assign(existing, payload);
      else state.treasureHunts.push({ id: uid('treasure'), ...payload });
      saveState(); closeModal(); renderAll(); showToast(existing ? 'Treasure Hunt record updated.' : 'Treasure Hunt recorded.');
    })
  });
}

function deleteTreasureHunt(recordId) {
  const item = (state.treasureHunts || []).find(record => record.id === recordId);
  if (!item || !confirm(`Delete the Treasure Hunt record for ${formatDate(item.date)}?`)) return;
  state.treasureHunts = state.treasureHunts.filter(record => record.id !== recordId);
  saveState(); renderAll(); showToast('Treasure Hunt record deleted.');
}

function getWelfareRecordsForPeriod() {
  return (state.welfareRecords || []).filter(recordMatchesPeriod).slice().sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')));
}

function renderWelfare() {
  const statsTarget = document.getElementById('welfareStats');
  const tableTarget = document.getElementById('welfareRecordsBody');
  if (!statsTarget || !tableTarget) return;
  const records = getWelfareRecordsForPeriod();
  const open = records.filter(item => item.status !== 'completed').length;
  const due = records.filter(item => item.nextFollowUpDate && item.status !== 'completed' && item.nextFollowUpDate <= isoToday()).length;
  const visits = records.filter(item => /visit/i.test(item.type || '')).length;
  statsTarget.innerHTML = [
    statCard('Welfare Records', records.length, 'Records in the selected period', '♡'),
    statCard('Open Follow-ups', open, 'Still requiring attention', '!'),
    statCard('Due Follow-ups', due, 'Follow-up date has arrived', '◷'),
    statCard('Visitation Records', visits, 'Recorded personal visits', '⌂')
  ].join('');

  tableTarget.innerHTML = records.length ? records.map(item => `<tr>
    <td><strong>${formatDate(item.date)}</strong></td>
    <td><strong>${escapeHTML(item.personName || '—')}</strong></td>
    <td>${escapeHTML(item.type || 'Welfare')}</td>
    <td><span class="table-note-clip">${escapeHTML(item.reason || '—')}</span></td>
    <td><span class="table-note-clip">${escapeHTML(item.action || '—')}</span></td>
    <td>${escapeHTML(item.assignedTo || '—')}</td>
    <td>${item.nextFollowUpDate ? formatDate(item.nextFollowUpDate) : '—'}</td>
    <td><span class="priority ${item.status === 'completed' ? 'watch' : item.status === 'urgent' ? 'high' : 'medium'}">${escapeHTML(item.status === 'completed' ? 'Completed' : item.status === 'urgent' ? 'Urgent' : 'Open')}</span></td>
    <td><div class="table-actions"><button class="mini-btn" data-welfare-edit="${item.id}">Edit</button><button class="mini-btn danger-mini" data-welfare-delete="${item.id}">Delete</button></div></td>
  </tr>`).join('') : '<tr><td colspan="9"><div class="empty-state">No visitation or welfare record has been added for this period yet.</div></td></tr>';
}

function openWelfareRecordForm(recordId = null) {
  const existing = (state.welfareRecords || []).find(item => item.id === recordId) || null;
  const memberNames = getCurrentMembers().slice().sort((a,b) => a.name.localeCompare(b.name)).map(member => `<option value="${escapeHTML(member.name)}"></option>`).join('');
  const shepherdNames = getShepherds().slice().sort((a,b) => a.name.localeCompare(b.name)).map(member => `<option value="${escapeHTML(member.name)}"></option>`).join('');
  openModal({
    eyebrow: 'Visitation & Welfare',
    title: existing ? 'Edit care record' : 'Add visitation / welfare record',
    subtitle: 'Use this for visitation, welfare assistance, absence follow-up, personal care, or another committee record that needs action.',
    body: `<form class="lux-form" id="welfareRecordForm">
      <datalist id="welfareMemberNames">${memberNames}</datalist><datalist id="welfareShepherdNames">${shepherdNames}</datalist>
      <div class="form-grid two-col">
        <label><span>Date *</span><input type="date" name="date" required value="${escapeHTML(existing?.date || isoToday())}"></label>
        <label><span>Person *</span><input name="personName" list="welfareMemberNames" required value="${escapeHTML(existing?.personName || '')}" placeholder="Member or visitor name"></label>
        <label><span>Record type</span><select name="type"><option ${existing?.type === 'Visitation' ? 'selected' : ''}>Visitation</option><option ${existing?.type === 'Welfare' ? 'selected' : ''}>Welfare</option><option ${existing?.type === 'Attendance follow-up' ? 'selected' : ''}>Attendance follow-up</option><option ${existing?.type === 'First-timer follow-up' ? 'selected' : ''}>First-timer follow-up</option><option ${existing?.type === 'General care' ? 'selected' : ''}>General care</option></select></label>
        <label><span>Assigned to</span><input name="assignedTo" list="welfareShepherdNames" value="${escapeHTML(existing?.assignedTo || '')}" placeholder="Shepherd / coordinator"></label>
        <label><span>Next follow-up date</span><input type="date" name="nextFollowUpDate" value="${escapeHTML(existing?.nextFollowUpDate || '')}"></label>
        <label><span>Status</span><select name="status"><option value="open" ${existing?.status !== 'completed' && existing?.status !== 'urgent' ? 'selected' : ''}>Open</option><option value="urgent" ${existing?.status === 'urgent' ? 'selected' : ''}>Urgent</option><option value="completed" ${existing?.status === 'completed' ? 'selected' : ''}>Completed</option></select></label>
      </div>
      <label><span>Reason / welfare need</span><textarea name="reason" rows="2" placeholder="Why was the visit or welfare follow-up needed?">${escapeHTML(existing?.reason || '')}</textarea></label>
      <label><span>Action taken</span><textarea name="action" rows="2" placeholder="What was done?">${escapeHTML(existing?.action || '')}</textarea></label>
      <label><span>Notes</span><textarea name="notes" rows="2" placeholder="Optional internal notes">${escapeHTML(existing?.notes || '')}</textarea></label>
      <div class="modal-actions"><button type="button" class="btn btn-soft" data-modal-cancel>Cancel</button><button type="submit" class="btn btn-gold">${existing ? 'Save changes' : 'Save record'}</button></div>
    </form>`,
    onOpen: () => document.getElementById('welfareRecordForm')?.addEventListener('submit', event => {
      event.preventDefault();
      const data = new FormData(event.currentTarget);
      const date = String(data.get('date') || '');
      ...
      const payload = {
        ...
        type: String(data.get('type') || 'Welfare'),
        assignedTo: String(data.get('assignedTo') || '').trim(),
        nextFollowUpDate: String(data.get('nextFollowUpDate') || ''),
      ...
      if (existing) Object.assign(existing, payload);
      else state.welfareRecords.push({ id: uid('welfare'), ...payload });
      saveState(); closeModal(); renderAll(); showToast(existing ? 'Welfare record updated.' : 'Welfare record saved.');
    })
  });
}

function deleteWelfareRecord(recordId) {
  const item = (state.welfareRecords || []).find(record => record.id === recordId);
  if (!item || !confirm(`Delete the ${item.type || 'welfare'} record for ${item.personName || 'this person'}?`)) return;
  state.welfareRecords = state.welfareRecords.filter(record => record.id !== recordId);
  saveState(); renderAll(); showToast('Welfare record deleted.');
}
...
function renderDashboard() {
  ...
  const careNavBadge = document.getElementById('careNavBadge');
  const visitorNavBadge = document.getElementById('visitorNavBadge');
  if (careNavBadge) careNavBadge.textContent = careMembers.length;
  if (visitorNavBadge) visitorNavBadge.textContent = getActiveVisitors().length;
}
...
document.getElementById('addTreasureHuntBtn')?.addEventListener('click', () => openTreasureHuntForm());
document.getElementById('addWelfareRecordBtn')?.addEventListener('click', () => openWelfareRecordForm());
...
document.addEventListener('click', event => {
  ...
  if (target.dataset.treasureEdit) return openTreasureHuntForm(target.dataset.treasureEdit);
  if (target.dataset.treasureDelete) return deleteTreasureHunt(target.dataset.treasureDelete);
  if (target.dataset.welfareEdit) return openWelfareRecordForm(target.dataset.welfareEdit);
  if (target.dataset.welfareDelete) return deleteWelfareRecord(target.dataset.welfareDelete);
